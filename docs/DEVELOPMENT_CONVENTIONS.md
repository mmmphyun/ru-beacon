...
- main은 항상 배포 가능한 Green 상태를 유지하며 1인 개발 트렁크 기반(Trunk-Based)으로 직결 커밋한다.
- 커밋 제목은 Conventional Commits 형식의 `<type>(<scope>): <한글 요약>`을 엄격히 준수한다. (마침표 금지, pre-commit/commit-msg 훅 강제)
- **독립 docs 커밋 원천 금지 및 원자적 결합(Co-located Commit)**: 문서(일지, 체크리스트 등)는 기능 구현(`feat`) 또는 리팩터링(`refactor`) 코드/테스트와 동일 커밋에 원자적으로 묶어야 하며, 단독 커밋 시 Git 훅에 의해 물리적으로 차단된다.
- **TDD 테스트 동시 커밋 강제**: 비즈니스 로직(`src/main/`) 추가/수정 시 반드시 검증 테스트(`src/test/`)가 동일 커밋에 포함되어야 한다.
- **단일 서브모듈 스코프 원칙**: 빅뱅 커밋을 방지하기 위해 단일 커밋에는 오직 1개 모듈의 변경만 포함한다. (멀티모듈 동시 스테이징 시 Git 훅 차단)
- 릴리스 배포는 `main` 직접 배포가 아닌 `vX.Y.Z` 태그 기반 CD 파이프라인 및 GitHub 환경 승인 게이트(Environment Protection Gate)를 통해 격발한다.
