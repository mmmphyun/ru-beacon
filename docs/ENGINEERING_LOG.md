...
#### 1. 아키텍처 트레이드오프 & 핵심 의사결정
- **인스턴스 인증: DB 저장 단방향 SHA-256 해시(선택) vs 무상태 JWT/HMAC 서명(기각)**:
  - *기각한 대안 (무상태 JWT/대칭키 토큰)*:
    - *기각으로 잃은 이익*: DB I/O 없이 CPU 서명 검증만으로 WSS 핸드셰이크를 즉시 통과시킬 수 있어 인그레스 부하 제로.
    - *기각한 이유*: 마인크래프트 서버 인스턴스가 탈취되거나 테넌트가 특정 서버 인스턴스를 즉각 퇴출(Revoke)해야 할 때, 블랙리스트 인프라나 만료 주기를 기다려야 하는 상태 불일치 발생. DB 조회는 핸드셰이크 최초 1회만 발생하므로, 단방향 SHA-256 해시를 DB에 보관하고 복합 인덱스 기반으로 1회 검증하는 방식이 즉각적인 폐기 권한과 보안 무결성을 완벽히 보장함.
- **WebSocket 인그레스와 Redis Streams 간 장애 격리: 예외 격리 세션 유지(선택) vs 세션 즉시 종료(기각)**:
  - *기각한 대안 (Redis 발행 실패 시 소켓 강제 종료)*:
    - *기각으로 잃은 이익*: 데이터 유실(이벤트 누락) 발생 시 즉시 연결을 끊어 클라이언트 측에 장애를 노출하고 재연결을 유도.
    - *기각한 이유*: WSS 채널은 단순 이벤트 수신뿐만 아니라 중앙에서 마인크래프트 서버로 긴급 제어 명령(`COMMAND_REQ`)을 내리는 핵심 통제 채널임. 일시적인 Redis 순단으로 인해 인게임 제어 채널까지 함께 파괴되는 것은 연쇄 장애(Cascading Failure)를 유발함. Redis 발행 예외는 격리하여 세션을 유지하고, 워커/스트림 복구 시 다시 정상 수용되도록 설계.

#### 2. AI 통제 및 거버넌스 (Human-in-the-Loop)
- **분산 소켓 재연결 레이스 컨디션 적발 및 원자적 세션 비교 교정**:
...
- **포니테일 기반 복잡도 사냥 (`/ponytail-review`)**:
  - `SessionRegistry.kt`에 무심코 선언되어 있던 미사용 `private val mutex = Mutex()` 삭제 (`delete`).
  - `InstanceAuthService.kt`의 조회 후 메모리 비교 로직을 SQL WHERE 절 결합 및 `.count() > 0` 단일 쿼리로 다이어트 (`shrink`).
