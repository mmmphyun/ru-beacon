...
- **멀티모듈 동시 커밋 차단**: 한 커밋에는 오직 1개 서브모듈의 변경만 포함되어야 한다. 2개 이상의 모듈이 스테이징되면 `.githooks/pre-commit` 훅에 의해 즉시 강제 거부된다.
- **TDD 테스트 동시 커밋 강제**: 비즈니스 로직(`src/main/`)을 추가/수정할 때는 반드시 해당 로직을 검증하는 테스트 코드(`src/test/`)가 동일 커밋에 함께 스테이징되어야 한다. (테스트 누락 시 `.githooks/pre-commit` 훅에 의해 즉시 강제 거부)
